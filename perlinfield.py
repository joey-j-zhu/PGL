...
class PerlinField:
    # do all shapes in x, y, s convention
    def __init__(self, image, cell_shape, starting_field=None):
        # Important info only includes target image and resolution
        self.image = image
        ...
        # Gradients represented by magnitude and direction. This is what our descent is concerned with
        self.mag = np.zeros((cell_shape[0] + 1, cell_shape[1] + 1))
        self.dxn = 2*np.pi*np.random.rand(cell_shape[0] + 1, cell_shape[1] + 1) # Direction initialized randomly
        # The actual gradients, used to increment magnitude and direction
        ...
        # Cache arrays for rendering image
        ...
        # Cache arrays for image fitting
        ...
        #print(self.main_y[4, 10])
        # For calculating gradient per cell
        ...
        # For rendering
        self.min_val = np.ones(self.shape) * -128
        #self.out = image
        self.max_val = np.ones(self.shape) * 128
    ...
    def render(self):
        ...
        grid = np.sqrt(2) * ((1 - t[:, :, 1]) * n0 + t[:, :, 1] * n1) + 1
    ...
    def compute_gradient(self):
        ...
        # Debugging purposes: here we can restrict some cells from descending
        def dump_cell(i, j, gradient):
            # Use magnitude bounding to keep vectors rotating at reasonable speeds
    ...
    def step(self, beta):
        ...
        # The gradient descent of magnitude is damped to avoid diverging to infinity, which is very easy
        # Pulled back to magnitude 1
        self.mag += self.mag_grad * beta * 0.00001
        self.mag += (np.ones((self.mag.shape)) - self.mag) * 0.2
        #print(self.mag[0, 0])
